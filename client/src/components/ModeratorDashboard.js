...
const ModeratorDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [pendingResponse, claimsResponse, statsResponse] =
        await Promise.all([
          api.get("/moderator/pending"),
          api.get("/moderator/claims"),
  ...
  const handleClaimAction = async (claimId, action) => {
    try {
      await api.patch(`/moderator/claims/${claimId}`, {
        action: action,
        moderatorId: "mod-001",
      });
        ...
        <div className="header-content">
          <div className="header-left">
            ...
            <p className="text-secondary">
              Review and moderate submitted items. Ensure quality and safety of
              all listings.
            </p>
          </div>
        </div>
      ...
      {activeTab === "items" && (
        <div className="card">
          ...
          {pendingItems.length === 0 ? (
            <div className="text-center p-6">
              <FiCheckCircle size={48} className="text-success mb-4" />
              <h3>All Caught Up!</h3>
              <p className="text-secondary">
      ...
      {activeTab === "claims" && (
        <div className="card">
          <div className="card-header">
            ...
            <p className="text-secondary">
              Review claim requests and decide whether to approve or reject
              them.
            </p>
          </div>
          ...
          {pendingClaims.length === 0 ? (
            <div className="text-center p-6">
              ...
              <p className="text-secondary">
                No claim requests are currently pending review.
              </p>
            </div>
          ) : (
            <div className="pending-items">
